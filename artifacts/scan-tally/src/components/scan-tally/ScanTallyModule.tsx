...
function ProgressRing({ value, max, accuracy }: { value: number; max: number; accuracy: number }) {
  ...
  return (
    <div className="relative flex h-24 w-24 shrink-0 items-center justify-center">
      ...
      <div className="absolute text-center">
        <div className="text-xl font-black tabular-nums leading-none">{accuracy}%</div>
        <div className="text-[10px] font-bold text-muted-foreground">accuracy</div>
      </div>
    </div>
  );
}
...
const TONE_CLASSES: Record<StatusKey | "Excess", { card: string; label: string; awb: string; reason: string; meta: string; value: string; divider: string; badge: string; badgeText: string }> = {
  ...
  Priority:          { card: "bg-red-50      border-red-300",     label: "text-red-700",     awb: "text-red-900",     reason: "text-red-800",     meta: "text-red-500",     value: "text-red-900",     divider: "border-red-200",     badge: "bg-red-600",     badgeText: "text-white"       },
...
function LatestScanCard({ latest }: { latest: ScanRecord | undefined }) {
  if (!latest) {
    return (
      <div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed bg-muted/20 p-8 text-center text-muted-foreground">
        <ScanSearch className="h-10 w-10 opacity-30" />
        <div className="text-sm font-bold">No scans yet</div>
        <div className="text-xs">Scan an AWB to see live status here</div>
      </div>
    );
  }

  const statusKey = latest.isPriority ? "Priority" : latest.status;
  const tone = TONE_CLASSES[statusKey] ?? TONE_CLASSES.Fresh;
  const Icon = STATUS_ICON[latest.isPriority ? "Priority" : latest.status] ?? Sparkles;
  const displayStatus = getDisplayStatus(latest.status, latest.isPriority);
  const isFresh = latest.status === "Fresh" && !latest.isPriority;
  const isFreshAging = isFresh && latest.daysInHub > 2;
  const isAging = isFreshAging || ((latest.status === "Undelivered" || latest.status === "Pending Attempt") && latest.attempts < 3 && latest.daysInHub > 2);

  return (
    <div className={cn("rounded-xl border-2 p-6 h-full", tone.card)}>
